...
-- 1) plan catalog: server-owned definitions for free/plus
...
-- 2) user_entitlements: per-user plan state (server-owned)
...
-- 3) profiles: per-user identity row + optional future settings sync
...
-- 4) weekly_summaries: weekly summary history + optional weekly note
create table if not exists public.weekly_summaries (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,

  -- Store the week anchor day (Monday in the app). Enforce one summary per week per user.
  week_start date not null,

  -- Maps to the app's computed fields:
  -- - pace      ~ avgPace
  -- - archetype ~ weekType
...
-- 5) note_memory: one synced note-memory row per user per day
...
-- 6) ai_usage: immutable per-request events (for quotas + abuse prevention)
...
-- Useful for daily usage lookups without adding a separate date column.
...
-- - client cannot forge/erase usage events
-- - service role bypasses RLS and can write safely

-- Automatically create a profile + free entitlement row for each new auth user.
